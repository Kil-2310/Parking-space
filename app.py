...
def create_app():
    ...
    async def route_get_clients() -> List[database_models.Client]:
        async with async_session() as session:
            async with session.begin():
                ...
                client = res.scalars().all()
                return client
                

    @app.get('/clients/{client_id}', status_code=200, response_model=schemas.BaseClientOut)
    async def route_get_client_by_id(client_id: int) -> database_models.Client:
        async with async_session() as session:
            async with session.begin():
                res = await session.execute(select(database_models.Client)
                                        .where(database_models.Client.client_id == client_id))
    ...
    async def route_create_parking_place(parking: schemas.BaseParkingIn) -> database_models.Parking:
        async with async_session() as session:
            async with session.begin():
    ...
    @app.put('/client_parkings', status_code=201, response_model=schemas.BaseClientParkingOut)
    async def create_client_parkings(client_parking: schemas.BaseClientParkingIn) -> database_models.ClientParking:
        async with async_session() as session:
            async with session.begin():
                parking_id = client_parking.parking_id

                result = await session.execute(
                    select(database_models.Parking)
                    .where(database_models.Parking.parking_id == parking_id)
                )
                ...
                db_client_parking = database_models.ClientParking(**client_parking.dict())
    ...
    @app.delete('/client_parkings', status_code=200, response_model=schemas.BaseClientParkingDelete)
